...
@click.away="dropdownOpen = false">
 ...
 type="button"
 class="flex items-center justify-center w-12 h-12 bg-zinc-50 text-zinc-400 hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 transition-colors relative"
 >
 ...
 <span class="absolute top-2 right-2 flex items-center justify-center w-4 h-4 text-[10px] font-bold text-white bg-red-500 border border-white dark:border-zinc-900">
 ...
 class="fixed inset-x-4 top-24 z-999 mx-auto w-[calc(100%-2rem)] max-w-sm flex-col border border-zinc-200 bg-white shadow-theme-lg dark:border-zinc-800 dark:bg-zinc-900 sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-3 sm:w-[380px]"
 style="display: none;"
 >
 <!-- Header -->
 ...
 @if($this->unreadCount > 0)
 <div class="flex items-center justify-center h-5 px-1.5 text-xs font-medium text-blue-700 bg-blue-100 dark:text-blue-100 dark:bg-blue-500/20">
 {{ $this->unreadCount }}
 </div>
 @endif
 </div>
 ...
 class="flex items-center justify-center w-7 h-7 text-zinc-500 hover:bg-zinc-200/50 dark:text-zinc-400 dark:hover:bg-zinc-700/50"
 type="button"
 >
 ...
 <div class="group flex items-center justify-between gap-3 p-1 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors relative">
 ...
 <img src="{{ Str::startsWith($notification->data['image'], 'http') ? $notification->data['image'] : Storage::url($notification->data['image']) }}" class="h-full w-full object-cover" alt="Image">
 @else
 <div 
 class="flex h-full w-full items-center justify-center {{ $notification->data['icon_bg'] ?? 'bg-zinc-100' }} {{ $notification->data['icon_color'] ?? 'text-zinc-500' }}"
 >
 ...
 class="opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-zinc-400 hover:text-red-500 hover:bg-zinc-100 dark:hover:bg-zinc-700 "
 ...
 <!-- Footer -->
 <div class="shrink-0 overflow-hidden bg-zinc-50 dark:bg-zinc-800 border-t border-zinc-100 dark:border-zinc-700">
 @if(count($this->filteredNotifications) > 0)
 <button 
