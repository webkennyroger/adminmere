        ...
        <div class="flex flex-col gap-2 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <button wire:click="create"
                    class="flex items-center justify-center bg-green-600 hover:bg-green-500 text-white rounded-lg h-9 px-4 py-2 text-sm font-medium transition-colors">
                    Novo Plano
                </button>
            </div>
        </div>
                        ...
                        @forelse($plans as $plan)
                            <tr wire:key="{{ $plan->id }}">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-center">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300">Inativo</span>
                                    @endif
                                </td>
                                <td class="px-4 py-4 text-sm font-medium text-center whitespace-nowrap">
                                    <div class="flex justify-center gap-2">
                                        <button wire:click="edit({{ $plan->id }})"
                                            class="inline-flex items-center justify-center gap-2 p-2 rounded-full text-green-600 bg-green-600/10 hover:bg-green-600/20">
                                        ...
                                        <button wire:click="confirmDelete({{ $plan->id }})"
                                            class="inline-flex items-center justify-center gap-2 p-2 rounded-full text-red-600 bg-red-600/10 hover:bg-red-600/20">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round"
                ...
                <form wire:submit="{{ $isEditMode ? 'update' : 'store' }}" class="space-y-6">
                    ...
                    <div class="flex items-center">
                        <input wire:model="is_active" type="checkbox" id="modal_is_active"
                            class="w-4 h-4 text-brand-600 bg-zinc-100 border-zinc-300 rounded focus:ring-brand-500 dark:focus:ring-brand-600 dark:ring-offset-zinc-800 focus:ring-2 dark:bg-zinc-700 dark:border-zinc-600">
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                        <button type="button" wire:click="closeModal"
                            class="px-4 py-2 bg-zinc-200 text-zinc-800 rounded-lg hover:bg-zinc-300 dark:bg-zinc-700 dark:text-zinc-200">Cancelar</button>
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-lg p-6 max-w-md w-full mx-4" wire:click.stop>
                ...
                <div class="flex justify-end gap-3 pt-5">
                    <button type="button" wire:click="$set('confirmingDeletion', false)"
                        class="px-4 py-2 bg-zinc-200 text-zinc-800 rounded-lg hover:bg-zinc-300">Cancelar</button>
                    <button wire:click="delete" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Sim,
                        Excluir</button>
                </div>
            </div>
        </div>
