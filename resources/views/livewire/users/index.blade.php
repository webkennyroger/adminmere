<div>
 ...
 class="bg-green-600 hover:bg-green-500 text-white h-9 px-4 py-2 text-sm font-medium transition-colors">
 Nova Usuário
 </button>
 </div>
 </div>
 ...
 <td class="px-6 py-4 whitespace-nowrap">
 <span
 class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium {{ ($user->profile?->plan ?? 'free') === 'free' ? 'bg-brand-50 text-brand-500 dark:bg-brand-500/20 dark:text-brand-400' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' }}">
 ...
 class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 {{ ($user->profile?->status ?? 'active') === 'active' ? 'bg-brand-600' : 'bg-zinc-200 dark:bg-zinc-700' }}"
 role="switch"
 aria-checked="{{ ($user->profile?->status ?? 'active') === 'active' ? 'true' : 'false' }}">
 <span class="sr-only">Use setting</span>
 <span
 class="pointer-events-none inline-block h-5 w-5 transform bg-white shadow ring-0 transition duration-200 ease-in-out {{ ($user->profile?->status ?? 'active') === 'active' ? 'translate-x-5' : 'translate-x-0' }}"></span>
 </button>
 </td>
 ...
 *</label>
 ...
 Perfil</label>
 @if ($image)
 <img src="{{ $image->temporaryUrl() }}" class="w-20 h-20 object-cover mb-2">
 @elseif ($currentImage)
 <img src="{{ $currentImage }}" class="w-20 h-20 object-cover mb-2">
 @endif
 ...
 class="px-4 py-2 bg-zinc-200 text-zinc-800 hover:bg-zinc-300">
 Cancelar
 </button>
