<div class="min-h-screen bg-zinc-50 dark:bg-black py-8">
 ...
 <!-- Header -->
 <div
 class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
 <div class="flex items-center gap-4">
 <a href="{{ profile_url($user) }}">
 <img src="{{ $user->image_url }}"
 class="w-16 h-16 border-4 border-white dark:border-zinc-800 shadow">
 </a>
 <div>
 ...
 Seguindo
 <span
 class="{{ $activeTab === 'following' ? 'bg-orange-100 text-orange-600' : 'bg-zinc-100 text-zinc-900' }} ml-3 py-0.5 px-2.5 text-xs font-medium inline-block">
 ...
 Seguidores
 <span
 class="{{ $activeTab === 'followers' ? 'bg-orange-100 text-orange-600' : 'bg-zinc-100 text-zinc-900' }} ml-3 py-0.5 px-2.5 text-xs font-medium inline-block">
 ...
 <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm p-6 flex flex-col items-center text-center transition-transform hover:-translate-y-1 duration-200"
 wire:key="user-{{ $relatedUser->id }}">

 <a href="{{ route('profile.view', $relatedUser) }}">
 <img src="{{ $relatedUser->image_url }}"
 class="w-24 h-24 border-4 border-white dark:border-zinc-800 shadow-md object-cover mb-4 hover:ring-2 hover:ring-brand-500 transition-all">
 </a>
 ...
 class="group w-full py-2 px-4 border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 text-sm font-bold hover:bg-yellow-500 hover:text-white hover:border-yellow-500 dark:hover:bg-yellow-600 transition-colors">
 <span class="block group-hover:hidden">Seguindo</span>
 <span class="hidden group-hover:block">Deixar de seguir</span>
 </button>
 @else
 <button wire:click="toggleFollow({{ $relatedUser->id }})" wire:loading.attr="disabled"
 class="w-full py-2 px-4 bg-green-600 text-white text-sm font-bold hover:bg-green-700 shadow-lg shadow-green-900/20 transition transform active:scale-95">
 Seguir
 </button>
 @endif
 @else
