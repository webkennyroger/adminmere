<div>
 ...
 class="p-2 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 focus:outline-none hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
 ...
 class="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-zinc-950 shadow-2xl border border-zinc-100 dark:border-zinc-800 z-50 py-2 overflow-hidden ring-1 ring-black/5"
 style="display: none;">
 
 <!-- Mark all as read -->
 <button wire:click="markAllAsRead" @click="openOptions = false" class="flex items-center w-full px-4 py-3 text-sm font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors gap-3 whitespace-nowrap">
 <svg class="w-5 h-5 text-zinc-500 dark:text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 ...
 <div class="relative bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
 ...
 class="relative group cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-all flex items-center shrink-0 p-2 gap-3 justify-start"
 title="{{ $user->name }}">
 ...
 alt="{{ $user->name }}"
 class=" object-cover ring-2 ring-transparent group-hover:ring-brand-500 transition-all w-10 h-10">
 ...
 <span class="flex items-center justify-center min-w-5 h-5 px-1.5 text-[10px] font-bold text-white bg-green-500 ">
 {{ $user->unread_count }}
 </span>
 @endif
 ...
 <span class="bg-zinc-100 dark:bg-zinc-800 text-zinc-500 text-[10px] px-1.5 ">{{ collect($groups)->count() }}</span>
 </div>
 ...
 <!-- Group Avatar -->
 <div class="relative shrink-0 w-10 h-10 overflow-hidden ring-2 ring-transparent group-hover:ring-blue-500 transition-all">
 ...
 class="shrink-0 flex items-center justify-center bg-green-50 text-green-600 hover:bg-green-100 transition-colors w-full py-2 gap-2"
 title="Nova Conversa">
 ...
 class="shrink-0 flex items-center justify-center bg-blue-50 text-blue-500 hover:bg-blue-100 transition-colors w-full py-2 gap-2"
 title="Criar Grupo">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <div class="bg-white dark:bg-zinc-950 w-full max-w-md shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
 ...
 <input wire:model="searchUser" type="text" placeholder="Digite o nome do usuário..." class="w-full px-3 py-2 border dark:bg-zinc-800 dark:border-zinc-700 dark:text-white focus:ring-brand-500" />
 <div class="mt-2 max-h-40 overflow-y-auto space-y-1">
 @foreach($filteredUsers as $user)
 <button wire:click="startChat({{ $user->id }})" class="w-full flex items-center gap-2 px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition">
 <img src="{{ $user->profile?->image ? Storage::url($user->profile->image) : $user->image_url }}" class="w-8 h-8 object-cover">
 ...
 <input wire:model="newGroupName" type="text" class="w-full px-3 py-2 border dark:bg-zinc-800 dark:border-zinc-700 dark:text-white focus:ring-brand-500" placeholder="Ex: Time de Vendas">
 ...
 <div class="space-y-2 border p-2 dark:border-zinc-700 max-h-48 overflow-y-auto">
 @foreach($users as $user)
 <label class="flex items-center gap-3 p-2 hover:bg-zinc-50 dark:hover:bg-zinc-800 cursor-pointer">
 <input type="checkbox" wire:model="selectedUsersForGroup" value="{{ $user->id }}" class=" text-brand-500">
 <img src="{{ $user->profile?->image ? Storage::url($user->profile->image) : $user->image_url }}" class="w-8 h-8 object-cover">
 <span class="text-sm dark:text-gray-200">{{ $user->name }}</span>
 </label>
 @endforeach
 ...
 <button wire:click="createGroup" class="px-4 py-2 text-sm bg-brand-500 text-white hover:bg-brand-600">Criar Grupo</button>
 </div>
 </div>
 </div>
 @endif
 ...
 <div class="bg-white dark:bg-zinc-950 w-full max-w-md shadow-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col max-h-[80vh]">
 <!-- Header -->
 ...
 <button wire:click="$set('showBlockedUsersModal', false)" class="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
 </button>
 </div>
 ...
 <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 group hover:border-zinc-200 dark:hover:border-zinc-700 transition-all">
 <div class="flex items-center gap-3">
 <div class="relative">
 <img src="{{ $blockedUser->image_url }}" class="w-10 h-10 object-cover ring-2 ring-white dark:ring-zinc-800">
 </div>
 <div>
 ...
 class="px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 dark:text-red-400 dark:bg-red-900/10 dark:hover:bg-red-900/20 transition-colors border border-red-100 dark:border-red-900/30">
 Desbloquear
 </button>
 </div>
 @empty
 <div class="flex flex-col items-center justify-center py-8 text-center">
 <div class="w-12 h-12 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center mb-3">
 ...
 <button wire:click="$set('showBlockedUsersModal', false)" class="w-full py-2.5 text-sm font-medium text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 shadow-sm hover:shadow transition-all">
 Fechar
 </button>
 </div>
 </div>
