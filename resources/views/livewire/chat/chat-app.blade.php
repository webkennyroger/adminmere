<div class="h-[calc(100vh-186px)] overflow-hidden sm:h-[calc(100vh-174px)]" x-data="{ 
 ...
 <!-- Chat List Side -->
 <div class="flex-col overflow-hidden border border-zinc-200 bg-white xl:flex xl:w-1/3 2xl:w-1/4 dark:border-white/5 dark:bg-zinc-900/50"
 ...
 class="absolute top-full right-0 z-40 w-40 mt-2 space-y-1 border border-zinc-200 bg-white p-2 shadow-xl dark:border-white/5 dark:bg-zinc-800"
 x-cloak>
 <button
 class="text-theme-xs flex w-full px-3 py-2 text-left font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-white/5 dark:hover:text-zinc-300">Ver
 mais</button>
 <button
 class="text-theme-xs flex w-full px-3 py-2 text-left font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-white/5 dark:hover:text-zinc-300 text-red-500">Excluir
 tudo</button>
 </div>
 </div>
 </div>
 ...
 class="h-11 w-full border border-zinc-200 bg-zinc-50 py-2.5 pl-11 pr-4 text-sm text-zinc-800 placeholder:text-zinc-400 focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 focus:outline-none dark:border-white/5 dark:bg-zinc-800 dark:text-white/90 dark:placeholder:text-white/30 transition-all">
 </div>
 </div>
 </div>
 ...
 <div wire:key="user-{{ $user->id }}" @click="selectMessageRoom({{ $user->id }})" class="group flex items-center gap-3 p-3 cursor-pointer transition-all duration-200
 ...
 <!-- Avatar -->
 <div
 class="h-full w-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-500 dark:text-zinc-300 font-bold overflow-hidden border border-zinc-200 dark:border-white/5">
 ...
 <!-- Status Dot -->
 <span
 class="absolute right-0.5 bottom-0.5 block h-3 w-3 border-2 border-white dark:border-zinc-900 {{ $user->id % 2 == 0 ? 'bg-green-500' : 'bg-zinc-300 dark:bg-zinc-600' }}"></span>
 </div>
 ...
 <span
 class="flex h-5 min-w-[20px] px-1.5 items-center justify-center bg-brand-500 text-[10px] font-bold text-white shadow-sm shadow-brand-500/20">
 {{ $user->unread_count }}
 </span>
 @endif
 </div>
 ...
 <div
 class="inline-flex h-12 w-12 items-center justify-center bg-zinc-100 dark:bg-zinc-800 text-zinc-400 mb-3">
 ...
 <!-- Chat Box Side -->
 <div class="flex h-full flex-col overflow-hidden border border-zinc-200 bg-white dark:border-white/5 dark:bg-zinc-900/50 xl:flex-1"
 :class="isMobile ? 'flex fixed inset-0 z-50 border-none' : 'hidden xl:flex'">
 ...
 <div
 class="h-10 w-10 sm:h-11 sm:w-11 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-600 dark:text-white font-bold overflow-hidden border border-zinc-200 dark:border-white/5">
 ...
 <button
 class="hidden sm:flex w-9 h-9 items-center justify-center bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 transition-all">
 ...
 class="w-9 h-9 flex items-center justify-center bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 transition-all">
 ...
 class="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-zinc-800 shadow-2xl border border-zinc-100 dark:border-white/5 z-50 overflow-hidden py-1">
 <button
 ...
 @if(!$isMe)
 <div
 class="h-7 w-7 bg-zinc-200 dark:bg-zinc-800 shrink-0 flex items-center justify-center text-[10px] font-bold text-zinc-600 dark:text-white border border-white dark:border-zinc-900 shadow-sm overflow-hidden mb-1">
 @if($selectedUser->profile?->image)
 <img src="{{ Storage::url($selectedUser->profile->image) }}"
 ...
 <!-- Message Bubble -->
 <div class="relative px-4 py-2.5 shadow-sm transition-all
 {{ $isMe
 ...
 class=" overflow-hidden border border-black/5 dark:border-white/10 bg-black/5 dark:bg-white/5">
 ...
 class="shrink-0 w-9 h-9 flex items-center justify-center shadow-sm transition-transform active:scale-95 {{ $isMe ? 'bg-white/20 text-white' : 'bg-brand-500 text-white' }}">
 ...
 <div
 class="h-1 w-full bg-black/10 dark:bg-white/10 overflow-hidden">
 ...
 class="text-[9px] font-extrabold px-1.5 py-0.5 {{ $isMe ? 'bg-white/20' : 'bg-zinc-100 dark:bg-zinc-700' }} transition-colors">
 ...
 <div class="p-2 bg-brand-500 text-white">
 ...
 class="p-2 hover:bg-black/5 dark:hover:bg-white/5 transition">
 ...
 <div
 class="flex-1 relative flex items-center bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-white/10 focus-within:border-brand-500 focus-within:ring-4 focus-within:ring-brand-500/10 transition-all p-1.5 sm:p-2">
 ...
 <span
 class="absolute -top-0.5 -right-0.5 flex h-4 w-4 items-center justify-center bg-brand-500 text-[9px] font-bold text-white shadow-sm ring-2 ring-zinc-50 dark:ring-zinc-900">{{ count($attachments) }}</span>
 @endif
 ...
 class="absolute inset-0 bg-white/80 dark:bg-zinc-900/80 flex items-center justify-center z-10"
 x-cloak>
 ...
 class="absolute inset-0 bg-white dark:bg-zinc-800 flex items-center px-4 gap-4 z-30 animate-in fade-in zoom-in duration-200"
 x-cloak>
 <div class="flex items-center gap-3 flex-1">
 <span
 class="flex h-3 w-3 bg-red-500 animate-pulse ring-4 ring-red-500/20"></span>
 ...
 class="h-full w-full flex items-center justify-center bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-brand-500 transition-all hover:scale-105 active:scale-95 shadow-sm border border-zinc-200 dark:border-white/5">
 <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2"
 viewBox="0 0 24 24">
 ...
 class="h-11 sm:h-12 w-11 sm:w-12 flex items-center justify-center bg-brand-500 text-white hover:bg-brand-600 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-brand-500/25 animate-in slide-in-from-right-4 duration-300">
 ...
 class="flex items-center gap-2 px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-white/5 shadow-sm group">
 ...
 <div
 class="w-32 h-32 bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center mb-8 relative">
 <div class="absolute inset-0 bg-brand-500/5 animate-ping opacity-25"></div>
