            ...
            <div class="px-4 py-3 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between bg-white dark:bg-zinc-950 shrink-0 h-14 z-20"
                ...
                <div class="flex items-center gap-3 overflow-hidden">
                    ...
                    <div class="relative shrink-0">
                        <div class="w-9 h-9 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-white/5 overflow-hidden flex items-center justify-center font-bold text-zinc-500">
                            @if($avatarUrl)
                ...
                <div class="flex items-center gap-0.5">
                    <button @click.stop="isMinimized = !isMinimized" class="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-400 dark:text-zinc-500 transition-all">
                    ...
                    <button wire:click="closeChat" class="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-500/10 text-zinc-400 transition-all">
            ...
            <div x-ref="chatContainer" 
                ...
                @forelse($chatMessages as $message)
                    ...
                    <div class="flex {{ $isMe ? 'justify-end' : 'justify-start items-end gap-2' }}" wire:key="popup-msg-{{ $message->id }}">
                        @if(!$isMe)
                            <div class="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-800 shrink-0 mb-1 border border-white dark:border-zinc-800 shadow-sm overflow-hidden flex items-center justify-center text-[8px] font-bold text-zinc-500">
                        ...
                        <div class="flex flex-col {{ $isMe ? 'items-end' : 'items-start' }} max-w-[80%]">
                            <div class="relative px-3 py-2 rounded-2xl text-[13px] shadow-sm font-medium
                                        {{ $isMe 
                                ...
                                @if(!empty($message->attachments))
                                    <div class="mb-2 space-y-1.5 last:mb-0">
                                        @foreach($message->attachments as $index => $at)
                                            <div class="rounded-lg overflow-hidden border border-black/5 dark:border-white/5">
                ...
                <div x-show="isTyping" class="flex gap-2 items-center" x-cloak>
                    <div class="bg-zinc-100 dark:bg-zinc-800 p-2 px-3 rounded-2xl flex items-center gap-1 shadow-sm">
                        <span class="w-1 h-1 bg-zinc-400 rounded-full animate-bounce"></span>
                        <span class="w-1 h-1 bg-zinc-400 rounded-full animate-bounce delay-75"></span>
                        <span class="w-1 h-1 bg-zinc-400 rounded-full animate-bounce delay-150"></span>
                    </div>
                </div>
                ...
                <form wire:submit.prevent="sendMessage" class="flex items-center gap-2"
                    ...
                    <div class="relative">
                        ...
                        <label for="popup-file" class="p-2 text-zinc-400 hover:text-brand-500 transition cursor-pointer flex items-center justify-center relative">
                            @if(count($attachments) > 0)
                                <span class="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-brand-500 text-white text-[8px] rounded-full flex items-center justify-center font-bold">{{ count($attachments) }}</span>
                            @endif
                    ...
                    <div class="flex-1 relative">
                        <input x-model="content" 
                               ...
                               placeholder="Digite..." 
                               class="w-full bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-white/5 rounded-full py-2 px-4 text-sm focus:ring-1 focus:ring-brand-500 dark:text-white transition-all">
                    ...
                    <button type="submit" 
                        class="bg-brand-500 hover:bg-brand-600 text-white p-2 rounded-full shadow-md transition-all active:scale-90 disabled:opacity-50"
