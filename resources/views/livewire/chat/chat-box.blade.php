<div>
    ...
    @if($isOpen && ($selectedUser || $selectedGroup))
            <div class="fixed bottom-0 z-60 w-[calc(100%-2rem)] md:w-96 bg-white dark:bg-zinc-900 shadow-2xl rounded-t-xl border-x border-t border-zinc-200 dark:border-zinc-800 flex flex-col transition-all duration-300 overflow-hidden"
                ...
                <!-- Header -->
                <div class="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-zinc-900 shrink-0 h-14"
                     @click="if(minimized) minimized = false">
                    <div class="flex items-center gap-3 overflow-hidden">
                        <!-- Back Button (Mobile only) -->
                        <button @click="$wire.closeChat()" class="md:hidden mr-1 text-zinc-400 hover:text-green-600 dark:hover:text-green-400 transition-colors">
                        ...
                        @if($selectedGroup)
                            ...
                            <div class="relative shrink-0">
                                <img src="{{ $selectedGroup->image_url ?? 'https://ui-avatars.com/api/?name=Group' }}" class="w-9 h-9 rounded-full object-cover ring-2 ring-white dark:ring-zinc-900">
                            </div>
                            <div class="flex flex-col min-w-0">
                                <h3 class="font-bold text-zinc-900 dark:text-zinc-100 text-sm truncate leading-tight">
                                    {{ $selectedGroup->name }}
                                </h3>
                                <span class="text-[10px] text-zinc-500 truncate" x-show="!minimized">Grupo</span>
                            </div>
                        @elseif($selectedUser)
                            <div class="relative shrink-0">
                            ...
                            <div class="flex flex-col min-w-0">
                                ...
                                <div class="flex items-center gap-1" x-show="!minimized">
                                     <span class="text-[10px] font-medium text-green-600 dark:text-green-400">Online</span>
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <div x-data="{ open: false }" class="relative" @click.stop>
                            <button @click="open = !open" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 dark:text-zinc-300 transition-colors">
                            ...
                            <div x-show="open" @click.outside="open = false" style="display: none;"
                                ...
                                class="absolute right-0 bottom-full mb-2 w-56 bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-zinc-100 dark:border-zinc-700 z-50 overflow-hidden py-1">
                         ...
                         <button @click.stop="minimized = !minimized" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 dark:text-zinc-300 transition-colors">
                            <!-- Minus Icon (Minimize) -->
                            <svg x-show="!minimized" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path></svg>
                            <!-- Plus/Expand Icon (Maximize) -->
                            <svg x-show="minimized" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V6a2 2 0 012-2h2M4 16v2a2 2 0 002 2h2m10-10V6a2 2 0 00-2-2h-2m4 10v2a2 2 0 01-2 2h-2"></path></svg>
                        </button>

                        <!-- Close Button -->
                        <button wire:click="closeChat" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20 transition-colors text-zinc-400">
