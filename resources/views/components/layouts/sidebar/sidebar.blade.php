...
<aside id="sidebar"
 class="fixed flex flex-col mt-0 top-16 xl:top-0 px-5 left-0 bg-white dark:bg-zinc-900 dark:border-zinc-800 text-zinc-900 h-[calc(100vh-4rem)] xl:h-screen transition-all duration-300 ease-in-out z-99999 border-r border-zinc-200"
 x-data="{
 openSubmenus: {},
 init() {
 ...
 <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
 class="mx-auto" src="{{ asset('assets/images/logo/logo-icon.svg') }}" alt="Logo" width="32"
 height="32" />
 </a>
 </div>
 ...
 <div>
 <!-- Menu Group Title -->
 <h2 class="mb-4 text-xs uppercase flex leading-[20px] text-zinc-400" :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ?
 ...
 @else
 <!-- Simple Menu Item -->
 <a href="{{ $item['path'] }}" class="menu-item group relative" :class="[
 ...
 <span
 class="animate-ping absolute inline-flex h-full w-full bg-red-400 opacity-75"></span>
 ...
 @if (!empty($item['new']))
 <span
 class="ml-2 inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-brand-500 text-white">
 new
 </span>
 @endif
 @if (!empty($item['badge']))
 <span
 class="ml-auto mr-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-xs font-bold bg-red-500 text-white">
 {{ $item['badge'] }}
 </span>
 @endif
 </span>
...
<div x-show="$store.sidebar.isMobileOpen" @click="$store.sidebar.setMobileOpen(false)"
 class="fixed left-0 top-16 z-40 h-[calc(100vh-4rem)] w-full bg-zinc-900/50 xl:hidden"></div>