...
<body x-data="{ 'loaded': true}" class="bg-gray-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 min-h-screen">
    ...
    <button @click="$store.chatSidebar.toggle()"
        ...
        class="w-12 h-12 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow-lg flex items-center justify-center transition-transform hover:scale-105 active:scale-95 group">
        ...
        @if(auth()->check() && auth()->user()->messagesReceived()->whereNull('read_at')->count() > 0)
            <span class="absolute top-3 right-3 w-3 h-3 bg-yellow-500 rounded-full border-2 border-green-600"
