...
<div class="rounded-xl border p-4 {{ $containerClass }}">
    <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
            @if($title)
                <h4 class="mb-1 text-sm font-semibold text-zinc-800 dark:text-white/90">
                    {{ $title }}
                </h4>
            @endif
            ...
            @if($showLink)
                <a 
                    href="{{ $linkHref }}" 
                    class="inline-block mt-3 text-sm font-medium text-zinc-500 underline dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300"
                >
                    {{ $linkText }}
                </a>
            @endif
