<div x-data="{
 ...
 <!-- Close Button -->
 <button @click="close()"
 class="absolute top-4 right-4 text-white hover:text-zinc-300 transition-colors p-2 z-50 bg-black/20 hover:bg-black/40">
 ...
 class="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:text-zinc-300 transition-colors p-2 bg-black/20 hover:bg-black/40 z-50">
 <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 ...
 class="absolute right-4 top-1/2 -translate-y-1/2 text-white hover:text-zinc-300 transition-colors p-2 bg-black/20 hover:bg-black/40 z-50">
 <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 ...
 class="max-w-full max-h-[90vh] object-contain shadow-2xl" autoplay></video>
 </template>
 <template
 x-if="images[currentIndex] && (typeof images[currentIndex] !== 'string' || !images[currentIndex].includes('.mp4'))">
 <img :src="images[currentIndex]" class="max-w-full max-h-[90vh] object-contain shadow-2xl">
 </template>
 </div>

 <!-- Counters -->
 <div x-show="images.length > 1"
 class="absolute bottom-4 left-1/2 -translate-x-1/2 text-white text-sm font-medium bg-black/40 px-4 py-1.5 z-50">
