...
<div x-data="{
 ...
 class="relative w-full {{ $maxWidth }} flex flex-col bg-white dark:bg-zinc-900 border-b border-zinc-200/80 dark:border-zinc-800 pointer-events-auto shadow-xl {{ $attributes->get('class') }}" 
 @click.stop>
 
 @if ($showCloseButton)
 <div class="absolute top-2 end-2 z-10">
 <button @click="open = false; $wire.dispatch('modal-closed')" type="button" class="size-8 inline-flex justify-center items-center gap-x-2 border border-transparent bg-gray-100 dark:bg-neutral-700 text-gray-800 dark:text-neutral-200 hover:bg-gray-200 dark:hover:bg-neutral-600 focus:outline-hidden focus:bg-gray-200 dark:focus:bg-neutral-600 disabled:opacity-50 disabled:pointer-events-none" aria-label="Close">
