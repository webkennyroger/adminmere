    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="w-full">
        ...
        <div class="flex justify-between flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 leading-5 rounded-md dark:bg-zinc-800 dark:border-zinc-700 dark:text-gray-400 opacity-50 cursor-not-allowed">
                    Anterior
                </span>
            @else
            ...
            @else
                <span class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-gray-500 bg-white border border-gray-300 leading-5 rounded-md dark:bg-zinc-800 dark:border-zinc-700 dark:text-gray-400 opacity-50 cursor-not-allowed">
                    Próxima
                </span>
            @endif
        </div>
        ...
        <div class="hidden sm:grid sm:grid-cols-3 sm:w-full sm:items-center">
            ...
            <div class="justify-self-start">
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" class="flex items-center gap-2 rounded-lg border border-zinc-300 bg-white px-3 py-3 text-theme-sm font-medium text-zinc-700 shadow-theme-xs hover:bg-zinc-50 hover:text-zinc-800 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400 dark:hover:bg-white/[0.03] dark:hover:text-zinc-200 sm:px-3.5">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M2.58301 9.99868C2.58272 10.1909 2.65588 10.3833 2.80249 10.53L7.79915 15.5301C8.09194 15.8231 8.56682 15.8233 8.85981 15.5305C9.15281 15.2377 9.15297 14.7629 8.86018 14.4699L5.14009 10.7472L16.6675 10.7472C17.0817 10.7472 17.4175 10.4114 17.4175 9.99715C17.4175 9.58294 17.0817 9.24715 16.6675 9.24715L5.14554 9.24715L8.86017 5.53016C9.15297 5.23717 9.15282 4.7623 8.85983 4.4695C8.56684 4.1767 8.09197 4.17685 7.79917 4.46984L2.84167 9.43049C2.68321 9.568 2.58301 9.77087 2.58301 9.99715C2.58301 9.99766 2.58301 9.99817 2.58301 9.99868Z" fill="currentColor"></path>
                        </svg>
                        <span class="hidden sm:inline">Anterior</span>
                            ...
                            @foreach ($element as $page => $url)
                                <li>
                                    @if ($page == $paginator->currentPage())
                                        <span class="flex h-10 w-10 items-center justify-center rounded-lg text-theme-sm font-medium bg-blue-500 text-white">
                                            {{ $page }}
                                        </span>
                                    @else
                                        <a href="{{ $url }}" class="flex h-10 w-10 items-center justify-center rounded-lg text-theme-sm font-medium text-zinc-700 hover:bg-blue-500/[0.08] hover:text-blue-500 dark:text-zinc-400 dark:hover:text-blue-500">
                                            {{ $page }}
                                        </a>
                                    @endif
                                </li>
